    ...
    <nav class="navbar navbar-expand navbar-light container">
      <div class="container-fluid p-0">
        ...
        <ul class="navbar-nav ml-auto">
          <router-link class="nav-link" to="/signup" v-if="!$store.state.isLoggedIn">{{
            $t("signUp")
          }}</router-link>
          <router-link class="nav-link" to="/login" v-if="!$store.state.isLoggedIn"
            >Login</router-link
          >
          <router-link class="nav-link" :to="'/user/'+$store.state.id" v-if="$store.state.isLoggedIn"
